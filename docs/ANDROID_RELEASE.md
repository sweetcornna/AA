...
AA Android 永久使用 `com.aa.expense`，只发布 release 签名的 `arm64-v8a` APK。当前版本为 `0.0.3`、`versionCode=3`。公开的 v0.0.2 使用 debug 证书，因此首次正式版需要先卸载；此后不得更换 application ID 或永久 release certificate。
...
`production-publish` 只保存 `ANDROID_CERT_SHA256` 并承担公开发布审批，不得获得 keystore 或 backend/provider secret。两个 environment 都启用 required reviewers。
...
- exact production origin 存在，staging/loopback/private HTTP/secret marker 不存在；
- 无 source map/native debug section。

## GitHub Actions：候选

从 GitHub Actions 选择已存在且 immutable 的 `v0.0.3` tag ref，手动运行 `.github/workflows/release.yml`：

- `operation=candidate`
- `tag=v0.0.3`（input 与 workflow run ref 必须指向同一 tag commit）
- `backend_fingerprint=<accepted production bundle SHA-256>`
- publish-only 输入全部留空。

workflow 会：

1. resolve exact tag commit，检查 `version=0.0.3`、`versionCode=3` 和 clean source，并拒绝 tag commit 的 `docs/PRIVACY.md` 仍含 `<REQUIRED>`；
...
metadata schema 2 只记录公开身份：repository、run ID、tag、commit、version、versionCode、backend fingerprint、application ID、ABI、APK filename/SHA-256、certificate SHA-256。
...
再次从同一 `v0.0.3` tag ref 手动运行同一 workflow：
